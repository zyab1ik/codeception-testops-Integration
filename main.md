...
* [А что было до?](#было)
    ...
    * [Делай два!](#Два)
    * [Делай три!](#Три)
* [Первая интеграция и первые результаты](#Интеграция)
* [Разочарование](#Разочарование)
* [Доработка напильником](#Напильник)
* [Финишная прямая](#Прямая)
* [Где ручные кейсы, Карл?](#Ручные)
* [Dashboards and reports](#Dashboards)
* [Заключение и планы на будущее](#Заключение)
...
Особенно болезненно будет тем, кто в проекте недавно и не очень хорошо представляет себе структуру тестового набора, и
...
То, что вы видите на скриншоте, это
адаптированный [стандарт документирования Javadoc](https://ru.wikipedia.org/wiki/%D0%93%D0%B5%D0%BD%D0%B5%D1%80%D0%B0%D1%82%D0%BE%D1%80_%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%86%D0%B8%D0%B8)
для PHP. Теперь посмотрим, как это выглядит в Allure:

На скриншоте видно, что тесты выстроены в структуре, а именно: Component -> Feature -> Story. Это предварительно
настраивается в самом Allure TestOps. В любой момент структуру можно отредактировать и дерево пересоберется в
соответствии с запросом. Теперь пора навести порядок с шагами в автотестах. Можно использовать метод executeStep как
надстройку и красивое описание для нашего метода. В нашем примере в сигнатуру метода первым параметром передаем текст,
который будет отображаться как human readable step в UI Allure для метода addInvite (пример одного их методов):
...
красивые и понятные названия. В итоге, тестовый сценарий в Allure выглядит вот так:
...
показывает сколько времени бежал самый долгий тест, график с длительностью тестов, — и в конце концов экспортировать
...
тест-кейсов по правилам и триггерам, например, только из ветки мастер. Работает это так: у нас есть автотест, в котором
...
Решение нашлось неожиданно: как-то вечером перед Гейзенбагом, одной из самых больших конференций по тестированию, я
просматривал программу прошлых докладов и обнаружил доклад на тему Test cases as code. Да, практика не новая, и про нее
...
executeStep. А так это выглядит в Allure:
...
тесты на тестировщиков, если вы пропишете правила в настройках запуска.
...
матрицу покрытия. Представьте себе единый график, на котором можно посмотреть, что для одной фичи у нас 3 ручных и 1
